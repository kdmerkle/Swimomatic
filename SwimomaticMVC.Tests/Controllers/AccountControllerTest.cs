...
using SwimomaticMVC;
using SwimomaticMVC.Controllers;
using SwimomaticMVC.Models;

namespace SwimomaticMVC.Tests.Controllers
{
    ...
    public class AccountControllerTest
    {
        ...
        public void ChangePassword_Get_ReturnsView()
        {
            ...
            //Assert.AreEqual(10, ((ViewResult)result).ViewBag.PasswordLength);
        }
        ...
        private class MockFormsAuthenticationService : IFormsAuthenticationService
        {
            ...
            public void SignIn(string LastName, bool createPersistentCookie)
            {
                // verify that the arguments are what we expected
                Assert.AreEqual("someUser", LastName);
        ...
        private class MockMembershipService : IMembershipService
        {
            ...
            public bool ValidateUser(string LastName, string password)
            {
                return (LastName == "someUser" && password == "goodPassword");
            }

            public MembershipCreateStatus CreateUser(string LastName, string password, string email)
            {
                if (LastName == "duplicateUser")
                {
